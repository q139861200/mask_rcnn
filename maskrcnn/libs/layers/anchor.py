...
def encode(gt_boxes, all_anchors, height, width, stride):
  ...
  if gt_boxes.size > 0:
      ...
      gt_assignment = overlaps.argmax(axis=1)  # (A) [.....]  长度为proposal_boxes
      max_overlaps = overlaps[np.arange(total_anchors), gt_assignment] #返回的是proposal对应最大的 gt_boxes的overlaps
      gt_argmax_overlaps = overlaps.argmax(axis=0)  # G  [.....] 长度为gt_boxes
      gt_max_overlaps = overlaps[gt_argmax_overlaps,  # gt_boxes对应 最大得proposal的 overlaps
      ...
      # if there is no gt
      labels[:] = 0
   #以上做的就是计算  gt_boxes和 proposal_anchors的overlaps,在overlaps大于阔值,就把labels赋值1否则为-1
...
def decode(boxes, scores, all_anchors, ih, iw):
  ...
  boxes = bbox_transform_inv(all_anchors, boxes) # 把 proposal anchors 用 (dx,dy,dw,dh) 预测成  groundtruth
  classes = np.argmax(scores, axis=1)
  scores = scores[:, 1] #只表示 前景得分
  final_boxes = boxes  
  final_boxes = clip_boxes(final_boxes, (ih, iw))
  classes = classes.astype(np.int32)# class  0表示bg,1表示fg
...
def _compute_targets(ex_rois, gt_rois): # ex_rois 所有anchors的数据,gt_rois 所有 gt_boxes的数据，且 长度是ex_rois,每一个数据对应anchors 跟 gt最大的那个gt_boxes数据
  ...
  return bbox_transform(ex_rois, gt_rois[:, :4]).astype(np.float32, copy=False) #计算的是 anchors跟 最大overlaps的gt_boxes直接的 bbox, 结果是dx,dy,dw,dh(x,y是中心点)
