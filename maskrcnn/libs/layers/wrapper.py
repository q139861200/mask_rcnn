...
def assign_boxes(gt_boxes, tensors, layers, scope='AssignGTBoxes'):
    ...
    with tf.name_scope(scope) as sc:
        min_k = layers[0]
        max_k = layers[-1]
        # 按照 gt_boxes 面积 分不同的等级 layers中的等级
        assigned_layers = \
            tf.py_func(assign.assign_boxes, 
                     [ gt_boxes, min_k, max_k ],
                     tf.int32)
        assigned_layers = tf.reshape(assigned_layers, [-1])# 对于每一个boxes都有一个 layers的indexes

        assigned_tensors = []
        for t in tensors: # [rois,batch_indx]  t=rois,t=batch_indx   循环两次
            split_tensors = []
            for l in layers: #[2,3,4,5]
                tf.cast(l, tf.int32)
            ...
            # assigned_tensors[ split_tensors[ IND=2 , IND=3,IND=4,IND=5]  , split_tensors[ IND=2,IND=3,IND=4.IND=5] ]
        return assigned_tensors + [assigned_layers]
            # [ assigned_tensors , assigned_layers ]   assigned_layers 每一个index号码都对应于 面积大小的等级