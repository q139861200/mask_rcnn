...
def assign_boxes(gt_boxes, min_k=2, max_k=5):
    ...
    if gt_boxes.size > 0:
        ...
        return k.astype(np.int32)  # 分类gt_boxes的面积，如果大于1，也就是max_k,那么 对应gt_boxes等级为5否则取4，3，
        # 小于最低值的取2
        # 其中 k = np.floor(k0 + np.log2(np.sqrt(areas) / 224))
         # inds = np.where(k < min_k)[0], 两个式子合并,可以算出areas大小
