...
def encode(gt_boxes, rois, num_classes):
  ...
  num_classes: scalar, number of classes, including  bg and other fg
  ...
  if gt_boxes.size > 0: 
      ...
      max_overlaps = overlaps[np.arange(rois.shape[0]), gt_assignment] # gt和max都是 proposal anchors的,maxoverlaps
      # 是每个anchors, 对应 最大的gt_boxes的值
...
#    背景有一个 阔值，前景也有一个阔值。 背景label是0  前景label是 具体类别。 每一个anchor对应最大的gt_boxes的overlaps 如果在
      # 其他在两个阔值中间的取 -1
      ...
      # labels是所有 符合阔值条件的bg和fg的rois ， bbox_targets 实际 是 [ 符合正负样本个数 ,  4 * numclass ]
      # 两个维度。  其中只有 具体的类别 才有数值(dxdydwdh) 也就是每一行只有  1/4有数值
      # bbox_inside_weights 也只有 具体那四个才有值
  ...
  return labels, bbox_targets, bbox_inside_weights   # 根据proposal anchors和gt_box 来计算出overlaps.  找出合理的 anchors
# 其中分为 bg和fg， 然后用gt_boxes和proposal anchor 编码出  dxdydwdh

def decode(boxes, scores, rois, ih, iw):  # 根据预测出来的 dxdydwdh , rois, scores 来输出 具体最终的 boxes（要经过原图大小的边缘裁剪）
  """Decode prediction targets into boxes and only keep only one boxes of greatest possibility for each rois
    Parameters
  ---------
  boxes: an array of shape (R, Kx4), [dx1, dy1, dw1, dh1, dx2, dy2, dw2, dh2] K is classes
  ...
  """
  boxes = bbox_transform_inv(rois, deltas=boxes) # 预测出的boxes [proposal anchors, 4 * classes]
  classes = np.argmax(scores, axis=1)  # scores也是预测出来的。
...
# 上面boxes的位移在每个anchor,每个种类中都有值boxes[batch, classes*4 ], 但最后classes 是最大的类别，且final_boxes内容也
# 只存储了最大类别的dxdydwdh, final_boxes[batch,4], classes[batch] 存储的是类别ID,  scores[batch]存储的是具体得分
def _compute_targets(ex_rois, gt_rois, labels, num_classes):
  ...
  targets = bbox_transform(ex_rois, gt_rois)  # targets
  clss = labels
...
#encode(gt_boxes, rois, num_classes)

gt_boxes = np.random.randint( 10,60,(10,2) )
s = np.random.randint( 20,40,(10,2) )
s = gt_boxes + s
classes = np.random.randint( 0,3,(10,1) )
gt_boxes = np.hstack((gt_boxes,s,classes))
noise = np.random.randint(-5,5,(10,4))
rois = gt_boxes[:,:4] + noise
overlaps,labels, bbox_targets, bbox_inside_weights   =  encode(gt_boxes,rois,3)
