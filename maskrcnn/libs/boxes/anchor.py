...
def anchors_plane(height, width, stride = 1.0, #stride = 2**5, scales=[2**3,2**4，2**5]
  ...
  anc = anchors(scales, ratios, base)  # 根据不同的面积和长宽生成  面积个数*长宽个数 的anchors
...
def generate_anchors(base_size=16, ratios=[0.5, 1, 2],
  ...
  ratio_anchors = _ratio_enum(base_anchor, ratios) # 根据面积不同来得到不同的anchors
  anchors = np.vstack([_scale_enum(ratio_anchors[i, :], scales)  # 单独取每个 anchor,其中 anchor的长宽有不同的scales
...
def _ratio_enum(anchor, ratios):
  ...
  w, h, x_ctr, y_ctr = _whctrs(anchor) #返回中心点和w,h
  size = w * h
...
if __name__ == '__main__':
  ...
  for i in range(10):
    ancs = anchors()
    all_anchors = cython_ancho1r.anchors_plane(200, 250, 4, ancs)
    num_anchors += all_anchors.shape[0] * all_anchors.shape[1] * all_anchors.shape[2]
    all_anchors = cython_ancho1r.anchors_plane(100, 125, 8, ancs)
    num_anchors += all_anchors.shape[0] * all_anchors.shape[1] * all_anchors.shape[2]
    all_anchors = cython_ancho1r.anchors_plane(50, 63, 16, ancs)
    num_anchors += all_anchors.shape[0] * all_anchors.shape[1] * all_anchors.shape[2]
    all_anchors = cython_ancho1r.anchors_plane(25, 32, 32, ancs)
    num_anchors += all_anchors.shape[0] * all_anchors.shape[1] * all_anchors.shape[2]
