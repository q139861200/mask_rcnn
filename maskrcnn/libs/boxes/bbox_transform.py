...
def bbox_transform_inv(boxes, deltas): #boxes is given boxes,用 预测出来得dxdydhdw来输出给boxes
    if boxes.shape[0] == 0:
...
def clip_boxes(boxes, im_shape):
    ...
    # x1 >= 0
    boxes[:, 0::4] = np.maximum(np.minimum(boxes[:, 0::4], im_shape[1] - 1), 0) # imshape存储的是图像像素个数。而boxes是从0开始计算的
    # y1 >= 0
