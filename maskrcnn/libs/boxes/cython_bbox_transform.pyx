...
def bbox_overlaps(np.ndarray[np.float_t,ndim=2] rois,np.ndarray[np.float_t,ndim=2] gt_boxes):
    cdef unsigned int N = rois.shape[0]
    cdef unsigned int G = gt_boxes.shape[0]
    cdef np.ndarray[np.float_t,ndim=2] res = np.zeros( (N,G),dtype=np.float )
    cdef  float roiA,gtA,unitA,dx1,dx2,dy1,dy2
    for i in range(N):
        roiA = (rois[i,2] - rois[i,0]+1.0)*  (rois[i,3] - rois[i,1]+1.0)
        for j in range(G):
            gtA = (gt_boxes[j,2] - gt_boxes[j,0] +1.0)*  (gt_boxes[j,3] - gt_boxes[j,1] +1.0)
            dx1 = max(rois[i,0],gt_boxes[j,0])
            dy1 = max(rois[i,1],gt_boxes[j,1])
            dx2 = min(rois[i,2],gt_boxes[j,2])
            dy2 = min(rois[i,3],gt_boxes[j,3])
            if dx1 > dx2 or dy1>dy2:
                res[i,j] = 0
            else :
                unitA = (dx2-dx1+1.0) * (dy2-dy1+1.0)
                res[i,j] =  unitA / ( gtA+roiA - unitA )
